...
std::istream& operator>>(std::istream& is, Train& ob)
{
	...
	std::cout << "Введите время на остановки (от 0,5 до 24 ) ";
	ob.time_stops=double_input(is,0.5,24);
	std::cout << "Введите количество вагонов (от 1 до 15 ) ";
	ob.wagon_count=number_input(is,1,15);
	std::cout << "Введите тип поезда ( на английском ) ";
	ob.train_type=word_input(is);
